function [Tb,Sb,wb] = meltrate_gamm_dep_new(Tm,Sm,Um,topofile,opt)
...
astar = (rhoref*cpM*gammaT-rhoI*cpI*Kit./H);
bstar = gammaS*rhoref*Lf;
cstar = rhoref*cpM*gammaT.*Tm+gammaS*rhoref*Lf+rhoI*cpI*Kit*Ts./H;
...
adash = astar * a;
bdash = astar * b + astar * c .* Pb - cstar;
cdash = bstar .* Sm;
Sb=(-bdash-sqrt(bdash.*bdash-4*adash.*cdash))./(2*adash);
Tb = a * Sb + b + c * Pb;
wb = gammaS .* (Sm - Sb)./Sb * y2s;
end
