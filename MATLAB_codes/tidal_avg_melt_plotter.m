...
%GL_ini = GL_ini/10^3;
GL_ini = 0;
plot((Y-200)/10^3,melt_tidal_avg,'k','LineWidth',2);
hold on;
...
hold on;
plot(Y/10^3,melt_p,'c','LineWidth',2);
%L = legend('$\dot{m}$ (GL moving)','GL (low tide)','GL (high tide)','GL (2015)','Flexure zone','$\dot{m}$ (Fixed GL)','Interpreter','latex');
L = legend('$\dot{m}$','GL (low tide)','GL (high tide)','No GZ','Interpreter','latex');
L.FontSize = 18;
L.FontWeight = 'bold';
print(gcf,'-dpng','-r300',fullfile(pwd,'melt_rate_tidally_averaged_seventh'));

inds = find(Y/10^3<-0.1);
melt_int = trapz(Y(inds)/10^3-0.2,melt_tidal_avg(inds)) * rho_I/rho_w;
