function [Tb,Sb,wb] = meltrate(Tm,Sm,topofile)
...
dz = max(dz);
h = hval * dz';
% astar = rhoI * cpI * Kit./(rhoref * Lf * H);
% bstar = rhoref * cpM * gammaT./(rhoref * Lf);
% cstar = rhoref/rhoI;
% d = astar .* (Ts - b - c * Pb) - bstar .* (b + c * Pb - Tm);
% L = a * (bstar + astar) * gammaS * cstar .* Sm;
% Sb = (-(d + gammaS * cstar) + sqrt((d + gammaS * cstar).^2 - 4 * L))./(2 * a * (bstar + astar));
% Sbind = find(Sb<0);
% if(~isempty(Sbind))
%     Sb = (-(d + gammaS * cstar) - sqrt((d + gammaS * cstar).^2 - 4 * L))./(2 * a * (bstar + astar));
% end
