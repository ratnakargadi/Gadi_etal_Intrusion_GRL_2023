function ploter(time_start,time_end)
...
clf;
ax1 = subplot(3,1,1);
contourf(ax1,Ymesh,Zmesh,mean.T',nlevels,'LineStyle','none');
colorbar;
caxis(ax1,[-0.3 0.3]);
colormap(ax1,'winter');
...
ax2 = subplot(3,1,2);
contourf(ax2,Ymesh,Zmesh,mean.S',nlevels,'LineStyle','none');
colorbar;
caxis(ax2,[34.5 34.75]);
colormap(ax2,'jet');
...
save(sprintf('Mean_%d_%d.mat',time_start,time_end),'mean');
end