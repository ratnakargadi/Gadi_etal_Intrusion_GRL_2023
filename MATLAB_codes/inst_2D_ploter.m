...
work_dir = pwd;
plot_dir = [work_dir filesep 'Conference_paper_new'];
if(~exist(plot_dir))
   mkdir(plot_dir);
end
...
%t_want = 6/24;
%% Co-ordinate values
Y = cumsum(squeeze(rdmds('DYC')))/km2m - 6.3;
...
%for i=length(list_T)
  ...
  %  hold on;
    ...
    %hold off;
    %p = get(gca, 'Position');
 %   h = axes('Parent', gcf, 'OuterPosition',[0.2277761416712 0.125701428571429 0.208455008683688 0.171395]);
    %[0.1303 0.1605 0.1935 0.333]);
    %contourf(h,Ymesh',Zmesh',T,'LineStyle','none');
 %   pcolorcen(Ymesh',Zmesh',T);colorbar;caxis(h,[Tmin Tmax]);colormap(h,'jet');
    %hold(h,'on');
...
%    ylim(h,[-480 -430]);
    ...
    %    V(1:nskip_hor:end,1:nskip_vesrt:end),W(1:nskip_hor:end,1:nskip_vert:end),scale_fac*nfac,'k')
    %quiver(h1,Ymesh',Zmesh',V,W,scale_fac*nfac,'k');
    %hold(h1,'off');
    %xlim(h1,[0 x_max*0.3]);
    %ylim(h1,[-500 -480]);
    xticks(ax1,[-6 0 10 20 30 40 50]);
    xlabel(ax1,'Distance from GL (km)');
    ylabel(ax1,'Depth (m)');
    ax1.TickLength = [0.05 0.035];
    ax1.LineWidth = 1;
    ax1.XAxis.MinorTick = 'on';
    ax1.XAxis.MinorTickValues = [-3 5:10:55];
    ax1.YAxis.MinorTick = 'on';
    ax1.YAxis.MinorTickValues = [-850:100:-50];
    %title(ax1,sprintf('Time = %s',time_vec));
